...
io.sockets.on('connection', function (socket) { // initial connection from a client
  ...
  watchBcastAnalytics();
  watchBcast('log', { level: 5, zone: 'io', eventCode: 'client', message: 'connection'}); // bcast connection count to 'watch' room
  watchBcast('log', { level: 4, zone: 'io', eventCode: 'server', message: 'payload', data: webApi.statusCounts() });
  socket.on('message', function (data) {
    watchBcast('log', { level: data.level || 2, zone: 'io', eventCode: 'client', message: 'message', data: data });
  });
  socket.on('partInfo', function (part) {
    watchBcast('log', { level: 2, zone: 'io', eventCode: 'client', message: 'partInfo', data: part });
  });
  ...
  socket.on('clientBroadcast', function (message) {
    ...
    try {
      ...
      watchBcast('log', { level: message.level || 4, zone: message.zone || 'server', eventCode: 'clientBroadcast', message: combinedMsg.join(':'), data: message.data });
    } catch (err) {
  ...
  socket.on('disconnect', function () { // client-server connection is closed
    ...
    watchBcastAnalytics();
    watchBcast('log', { level: 5, zone: 'io', eventCode: 'client', message: 'disconnect'});
  });
});
